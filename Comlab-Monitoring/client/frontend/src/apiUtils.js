...
export const scanLabQRCode = async (labNumber) => {
  try {
    const response = await axios.post('http://localhost:8000/api/labs/scan-lab', { labNumber });
...
export const scanInstructorQRCode = async (labNumber, instructorName) => {
  try {
    const response = await axios.post('http://localhost:8000/api/labs/scan-instructor', {
      labNumber,
      instructorName,
    });
    return response.data;
